...
from ..services.auth_service import register_user, login_user, register_vendor
...
# <------------ Vendor Registration --------->
@router.post('/register/vendor', status_code=status.HTTP_201_CREATED, response_model=UserCreateResponse)

def vendor_registration(user: UserCreate  = Body(...), db: Session = Depends(get_db)):
   return register_vendor(db, user)
